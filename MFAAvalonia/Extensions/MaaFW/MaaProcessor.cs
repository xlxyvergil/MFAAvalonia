...
// 防止 interface 加载失败时 Toast 重复显示
        ...
        foreach (var selectOption in optionsToProcess)
        {
            ...
            else if (interfaceOption.IsScanSelect)
            {
                ...
                if (interfaceOption.Cases == null || interfaceOption.Cases.Count == 0)
                {
                    ...
                    {
                        ScanDirectoryForOption(interfaceOption);
                    }
                }

                if (selectOption.Index is int scanIndex
                    && interfaceOption.Cases is { } scanCases
                    && scanIndex >= 0
                    && scanIndex < scanCases.Count)
                {
                    var selectedCase = scanCases[scanIndex];
                    var selectedValue = selectedCase.Name ?? string.Empty;

                    // 使用 GenerateScanSelectProcessedPipeline 处理 pipeline_override 中的占位符
                    if (interfaceOption.PipelineOverride != null)
                    {
                        var processedPipeline = interfaceOption.GenerateScanSelectProcessedPipeline(selectedValue);
                        if (!string.IsNullOrWhiteSpace(processedPipeline) && processedPipeline != "{}")
                        {
                            var param = JsonConvert.DeserializeObject<Dictionary<string, JToken>>(processedPipeline);
                            if (param != null)
                            {
                                taskModels.Merge(param);
                            }
    ...
    /// <summary>
    /// 为 scan_select 类型的选项扫描目录
    /// </summary>
    private void ScanDirectoryForOption(MaaInterface.MaaInterfaceOption interfaceOption)
    {
        ScanSelectUtils.ScanDirectory(interfaceOption, ResourceBase);
    }
